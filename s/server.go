...
const (
	AddDataConn uint8 = 0x01 //通知客户端呼入一个数据通道
)
...
func (pS *ProxyServer) handleWeb(webClient *net.TCPConn) {
	...
	//通知客户端创建一个新的数据通道
	pS.pClient.cmdConn.Write([]byte{AddDataConn})
	dataClient, _ := pS.listener.dataListener.AcceptTCP()
	...
	go pcTos.encryptCopy(dataClient, webClient) //客户端收到的是明文，编码后就成了密文并传给代理的服务端
}
